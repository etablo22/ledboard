	...
	switch(type) {
		case 0: {
			...
			PORT_RS485 |= (1 << RXD);	//подтягивающий резистор на линии RXD для снижения помех
				...
				UCSR0B |= (1 << TXCIE0);	//разрешение прерывания по окончанию передачи TX
				MCUCR &= ~(1 << PUD);		//включение внутренних подтягивающих резисторов на портах PUD (Pull-Up Disable)
			#elif	(CPU_TYPE == ATMEGA8)
				//for Atmega8A
				UCSRB |= (1 << TXCIE);		//разрешение прерывания по окончанию передачи TX
				SFIOR &= ~(1 << PUD);		//включение внутренних подтягивающих резисторов на портах PUD (Pull-Up Disable)
			#endif
...
#if ( (CPU_TYPE == ATMEGA328) || (CPU_TYPE == ATMEGA88) )
	//for Atmega88 and Atmega328
	OCR2A = BriLevels[BriMode];		//установка яркости по выбранному режиму
#elif	(CPU_TYPE == ATMEGA8)
	//for Atmega8A
...
void(* resetCPU) (void) = 0; // Функция программного сброса МК
...
void Initialize(void)
{
	...
	readEepData();//проверяем целостность данных еепром
...
//проверяем целостность данных еепром
void readEepData() {
	...
	for (uint8_t i = 0; i < 4; i++) {
		...
		eeprom_write_byte(EEDevPinCode + i, DigTmp[i] % 10);
	}
	if (stelaTabPosition > 9) {
		eeprom_write_byte(EETab, 101);
		resetCPU();
	}
}
...
void ColorButtonsClick(uint8_t shiftVal)
{
	...
	//сбрасываем точки
	Digit[0] = Digit[0] & 0x7F;
	Digit[1] = Digit[1] & 0x7F;
	Digit[2] = Digit[2] & 0x7F;
	...
	Digit[0] = Digit[0] | ((maskDegVal << 7) & 0x80);		//сложно обойтись без сдвига, т.к. с разными масками устанавливаются разные биты
	Digit[1] = Digit[1] | ((maskDegVal << 6) & 0x80);
